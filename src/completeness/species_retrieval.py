...
def retrieve_species_n_gram(trace: list[dict[str, Any]], n: int) -> List[str]:
    if len(trace) < n:
        return ["NULL"]
    events: List[str] = [x.get("concept:name") for x in trace]
...
def retrieve_species_trace_variant(trace: list[dict[str, Any]]):
    return [",".join([x.get("concept:name") for x in trace])]


def retrieve_timed_activity(trace: list[dict[str, Any]], interval_size):
    l = []
...
def retrieve_timed_activity_exponential(trace: list[dict[str, Any]]):
    l = []
    if len(trace) == 1:
