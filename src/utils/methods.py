...
class ConformanceMetrics:
    ...
    def f1_score(self) -> float:
        return 2 * (self.precision * self.fitness) / (self.precision + self.fitness)

    def __str__(self):
        return (f"F1: {self.f1_score:.2f}, F: {self.fitness:.2f}, P: {self.precision:.2f}, G: {self.generalization:.2f}, "
...
def group_traces_by_case_id_dataframe(df: pd.DataFrame, key: str = "case:concept:name") -> pd.DataFrame:
    ...
    grouped_data: List[Dict[str, Any]] = [{"group": group_key, "traces": grouped_dict[group_key]} for group_key in order]
