...
class EstimatorWindow(AdaptiveWindow):
    def __init__(self,completeness_threshold: float, on_window: Callable[[List[Dict[str, Any]]], None]) -> None:
        super().__init__(on_window)
    ...
    def observe_event(self, event: Dict[str, Any], index: Optional[int]) -> None:
        ...
        estimator.profile_log(self.get_window())

        # estimated: float = getattr(estimator, "coverage_abundance")[0]
        estimated: float = getattr(estimator, "completeness_incidence")[0]

        if len(self.memory) <= 5:
            estimated = 0.0

        if estimated >= self.completeness_threshold:
            self.on_window(self.get_window(), index)
            self.memory.clear()
